...
*SQL Injection – внедрение SQL-кода. Существование данного типа атак обусловлено недостатками логики работы веб-приложения, позволяющего передать в систему управления базой данных SQL-подобный запрос без какого-либо механизма авторизации, либо если в самом приложении некорректно обрабатываются входные данные, используемые в SQL-запросах. Внедрение SQL, в зависимости от типа используемой СУБД и условий внедрения, может дать возможность атакующему выполнить произвольный запрос к базе данных, получить возможность чтения и/или записи локальных файлов и выполнения произвольных команд на атакуемом сервере.*

*При проведении данного типа атаки злоумышленник пытается в запросах к веб-приложению передать какой-либо SQL-запрос (например, вставка/обновление/удаление данных). В SQL-запрос к базе данных включаются данные, меняющие логику выполнения запроса таким образом, чтобы обойти должную авторизацию для выполнения команды или доступа к информации. В результате возникает возможность компрометации, несанкционированной модификации данных или их удаления.*
